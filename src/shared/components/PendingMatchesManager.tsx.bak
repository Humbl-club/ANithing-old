    ...
    try {
      ...
      setPendingMatches(
        (data || []).map((item: any) => ({
          ...
          title_japanese: item.title_japanese ?? undefined,
          synopsis: item.synopsis ?? undefined,
          image_url: item.image_url ?? undefined,
          score: item.score ?? undefined,
          year: item.year ?? undefined,
          admin_decision: item.admin_decision ?? undefined,
          potential_matches: Array.isArray(item.potential_matches) 
            ? (item.potential_matches as any[]).map(pm => ({
                ...pm,
                title_english: pm.title_english ?? undefined,
                title_japanese: pm.title_japanese ?? undefined
              }))
            : []
        })) as PendingMatch[]
      );
