    ...
    const fetchData = async () => {
      try {
        ...
        if (pageType && pageType !== 'attributions') {
          ...
          if (pageData) {
            const normalized: LegalPage = {
              id: pageData.id,
              page_type: pageData.page_type || 'unknown',
              title: pageData.title || 'Untitled',
              content: pageData.content || '',
              version: pageData.version ?? null,
              effective_date: pageData.effective_date ?? null,
              last_updated: pageData.last_updated ?? null,
            };
            setLegalPage(normalized);
          }
        }
        ...
        setApiAttributions((attributionsData || []).map((a: any) => ({
          id: a.id,
          service_name: a.service_name || 'Unknown',
          attribution_text: a.attribution_text || '',
          license_url: a.license_url ?? null,
          terms_url: a.terms_url ?? null,
        })));
      } catch (err: any) {
          ...
          <Card>
            <CardHeader>
              ...
              <div className="text-sm text-muted-foreground">
                Version {legalPage.version ?? '1.0'} • 
                Last updated: {new Date(legalPage.last_updated || Date.now()).toLocaleDateString()}
              </div>
            </CardHeader>
            <CardContent>
