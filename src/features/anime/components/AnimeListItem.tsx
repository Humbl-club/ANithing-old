...
import { FillerIndicator } from '@/features/anime/components/FillerIndicator';
...
const AnimeListItemComponent = ({
  ...
  const progressInfo = useMemo(() => 
    getMainStoryProgress(entry.episodes_watched || 0),
    [getMainStoryProgress, entry.episodes_watched]
  );
  
  const nextMainStoryEp = useMemo(() => 
    getNextMainStoryEpisode(entry.episodes_watched || 0),
    [getNextMainStoryEpisode, entry.episodes_watched]
  );
  
  const currentEpIsFiller = useMemo(() => 
    entry.episodes_watched ? isFillerEpisode(entry.episodes_watched) : false,
    [entry.episodes_watched, isFillerEpisode]
  );

  const handleSkipToMainStory = useCallback(() => {
    if (nextMainStoryEp) {
      onUpdate(entry.id, { episodes_watched: nextMainStoryEp });
    }
  }, [nextMainStoryEp, onUpdate, entry.id]);
