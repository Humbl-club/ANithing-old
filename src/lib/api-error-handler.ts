...
export const setupApiErrorTracking = () => {
  const originalFrom = supabase.from.bind(supabase) as any;
  (supabase as any).from = (table: any) => {
    const query = originalFrom(table);
    const originalSelect = query.select.bind(query) as any;
    const originalInsert = query.insert.bind(query) as any;
    const originalUpdate = query.update.bind(query) as any;
    const originalDelete = query.delete.bind(query) as any;
