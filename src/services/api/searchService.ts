...
class SearchApiService extends BaseApiService {
    ...
    try {
      ...
      if (type === 'anime' || type === 'both') {
        ...
        if (animeError) {
          ...
          results.anime = (animeData as DatabaseSearchAnimeResponse[])?.map((item: DatabaseSearchAnimeResponse) => {
      ...
      if (type === 'manga' || type === 'both') {
        ...
        if (mangaError) {
          ...
          results.manga = (mangaData as DatabaseSearchMangaResponse[])?.map((item: DatabaseSearchMangaResponse) => {
  ...
  async advancedSearch(options: {
    ...
    limit?: number;
    page?: number;
  }): Promise<ServiceResponse<{ data: (AnimeContent | MangaContent)[], pagination: { total: number; page: number; limit: number; hasMore: boolean } }>> {
    try {
      const {
        query,
        genres,
      ...
      return this.handleError(error, 'perform advanced search');
    }
  }
}
